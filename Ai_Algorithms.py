...
class AiAlgorithms:
    ...
    def minimax(self, board, is_max, ai_symbol, depth=0, metrics=None):
        if metrics is None:
            metrics = {'moves': 0}
        ...
        if is_max:
            ...
            for move in self.all_possible_moves(board):
                ...
                rec_move, rec_result = self.minimax(board, False, ai_symbol, depth + 1, metrics)
            ...
            for move in self.all_possible_moves(board):
                ...
                rec_move, rec_result = self.minimax(board, True, ai_symbol, depth + 1, metrics)
                board[move['row']][move['col']] = None
                if rec_result < best_score:
                    best_score = rec_result
                    best_move = dict(move)
    ...
    def alpha_beta(self, board, alpha, beta, is_max, ai_symbol, metrics=None):
        if metrics is None:
            metrics = {'moves': 0}
        best_move = {'row': -1, 'col': -1}
        is_over, result = self.is_game_over(board)
        ...
        if is_max:
            ...
            for move in self.all_possible_moves(board):
                metrics['moves'] += 1
                board[move['row']][move['col']] = ai_symbol
                rec_move, rec_result = self.alpha_beta(board, alpha, beta, False, ai_symbol,metrics)
                board[move['row']][move['col']] = None
            ...
            for move in self.all_possible_moves(board):
                ...
                # all_possible_moves[{row:1,col:3},]
                board[move['row']][move['col']] = 'o' if ai_symbol == 'x' else 'x'
                rec_move, rec_result = self.alpha_beta(board, alpha, beta, True, ai_symbol,metrics)
                board[move['row']][move['col']] = None
    ...
    def alpha_beta_symmetry(self, board, depth, alpha, beta, is_max, ai_symbol, metrics=None):
        if metrics is None:
            metrics = {'moves': 0}
        best_move = {'row': -1, 'col': -1}
        is_over, result = self.is_game_over(board)
        ...
        if is_max:
            ...
            for move in self.symmetry_reduction(board):
                metrics['moves'] += 1
                board[move['row']][move['col']] = ai_symbol
                rec_move, rec_result = self.alpha_beta_symmetry(board, depth + 1, alpha, beta, False, ai_symbol, metrics)
                board[move['row']][move['col']] = None
                alpha = max(alpha, rec_result)
                if rec_result > best_score:
                    best_score = rec_result
            ...
            for move in self.symmetry_reduction(board):
                ...
                # all_possible_moves[{row:1,col:3},]
                board[move['row']][move['col']] = 'o' if ai_symbol == 'x' else 'x'
                rec_move, rec_result = self.alpha_beta_symmetry(board, depth + 1, alpha, beta, True, ai_symbol, metrics)
                board[move['row']][move['col']] = None
                beta = min(beta, rec_result)
                if rec_result < best_score:
                    best_score = rec_result
    ...
    def minimax_heuristic_reduction(self, board, depth, is_max, ai_symbol,metrics=None):
        if metrics is None:
            metrics = {'moves': 0}
        best_move = {'row': -1, 'col': -1}
        is_over, result = self.is_game_over(board)
        ...
        if is_max:
            ...
            for move in all_moves:
                metrics['moves'] += 1
                board[move['row']][move['col']] = ai_symbol
                rec_move, rec_result = self.minimax_heuristic_reduction(board, depth + 1, False, ai_symbol, metrics)
                board[move['row']][move['col']] = None
                if rec_result > best_score:
                    best_score = rec_result
                    # using dict for shallow copy
            ...
            for move in all_moves:
                ...
                # all_possible_moves[{row:1,col:3},]
                board[move['row']][move['col']] = 'o' if ai_symbol == 'x' else 'x'
                rec_move, rec_result = self.minimax_heuristic_reduction(board, depth + 1, True, ai_symbol, metrics)
                board[move['row']][move['col']] = None
                if rec_result < best_score:
                    best_score = rec_result
                    # using dict for shallow copy
